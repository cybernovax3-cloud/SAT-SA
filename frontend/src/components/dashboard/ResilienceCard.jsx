...
const ResilienceCard = ({ score }) => {
  const roundedScore = Math.round(score || 0);

  // Resilience: Higher is better (Success style). Lower is worse (Danger style).
  const getSeverityStyle = (val) => {
    if (val >= 80) return { label: 'HIGH', color: 'var(--success)', bg: 'rgba(16, 185, 129, 0.05)' };
    if (val >= 50) return { label: 'MEDIUM', color: 'var(--warning)', bg: 'rgba(245, 158, 11, 0.05)' };
    return { label: 'LOW', color: 'var(--danger)', bg: 'rgba(239, 68, 68, 0.05)' };
  };
