...
import { getRisk, readRouteData, writeRouteData, isDemoModeEnabled } from '../api/api';
import { mockRiskAnalytics } from '../data/mockData';

export const useRisk = (pollingIntervalMs = 5000) => {
  const cacheKey = 'risk:data';
  const initialData = readRouteData(cacheKey, isDemoModeEnabled() ? mockRiskAnalytics : null);
  ...
  const load = useCallback(async (manual = false) => {
    ...
    try {
      const response = await getRisk(100);
    ...
    } catch (requestError) { setStatus('OFFLINE'); setError(requestError.message || 'Unable to retrieve risk analytics.'); }
